(() => {
  ...
  [searchInput, statusFilter, verificationFilter, roleFilter]
    .filter(Boolean)
    .forEach((control) => {
  ...
  const uploadProfileImage = async (file) => {
    ...
    if (!response.ok)
      throw new Error(body.message || "Unable to upload the profile image.");
  ...
  document
    ...
    .forEach((button) =>
      button.addEventListener("click", () => showModal(button.closest("tr"))),
    );
    ...
    button.addEventListener("click", async () => {
      ...
      try {
        ...
        if (tbody && !tbody.querySelector("tr[data-id]")) {
          tbody.innerHTML =
            '<tr><td class="empty-state" colspan="8">No user accounts found.</td></tr>';
        }

        document.querySelectorAll(".recent-user").forEach((card) => {
          if (
            row?.dataset.email &&
            card.textContent.toLowerCase().includes(row.dataset.email)
          ) {
            card.remove();
          }
        });
        ...
        const totalUsersElem = document.querySelector(
          ".user-stats-grid article:first-child strong",
        );
        if (totalUsersElem) {
