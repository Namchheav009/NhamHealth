...
public class AiFoodAnalysisService {
        ...
        private static final String DISCLAIMER = "Nutrition is calculated from matched database foods when available; unmatched components may use a clearly labeled AI estimate. Results are for general wellness only, not medical advice or an official nutrition label.";
        private static final String PRIVACY_NOTICE = "Your food image is sent to the configured AI provider for recognition. Do not include faces, documents, or other personal information.";
        ...
        public AiFoodAnalysisResponse analyzeAndSave(
                ...
                analysis.setStatus(!result.foodDetected()
                                ? "FAILED"
                                : result.needsUserConfirmation() ? "NEEDS_CONFIRMATION" : "COMPLETED");
        ...
        public void saveFeedback(Integer userId, Integer analysisId, AiFoodFeedbackRequest request) {
                ...
                analysis.setCorrectedServingSize(request.servingSize() == null
                                ? null
                                : request.servingSize().setScale(2, java.math.RoundingMode.HALF_UP));
