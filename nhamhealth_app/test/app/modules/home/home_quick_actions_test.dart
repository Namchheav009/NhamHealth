...
void main() {
  ...
  testWidgets('HomeQuickActions renders all 4 core feature action buttons', (
    tester,
  ) async {
    ...
    expect(
      find.byKey(const ValueKey('home-action-explore-meals')),
      findsOneWidget,
    );
    expect(find.text('Find Meals'), findsOneWidget);
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(find.text('រកមុខម្ហូប'), findsOneWidget);
  ...
  testWidgets(
    'NutritionProgressCard displays remaining budget and over budget state',
    (tester) async {
      ...
      expect(find.text('800 left'), findsOneWidget);
      ...
      expect(find.text('200 over'), findsOneWidget);
    },
  );

  testWidgets(
    'HomeView integrates HomeQuickActions directly under TimeGreeting',
    (tester) async {
      tester.view.devicePixelRatio = 1;
      ...
      expect(
        find.byKey(const ValueKey('home-action-scan-food')),
        findsOneWidget,
      );
      expect(
        find.byKey(const ValueKey('home-action-log-water')),
        findsOneWidget,
      );
      expect(
        find.byKey(const ValueKey('home-action-meal-plan')),
        findsOneWidget,
      );
      expect(
        find.byKey(const ValueKey('home-action-explore-meals')),
