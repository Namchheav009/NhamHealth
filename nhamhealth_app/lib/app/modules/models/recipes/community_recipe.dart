class CommunityRecipe {
  ...
  final bool saved;
  factory CommunityRecipe.fromJson(
    ...
    ingredients:
        (json['ingredients'] as List<dynamic>? ?? const [])
            .map(
              (value) => RecipeIngredient.fromJson(
    ...
    steps:
        (json['steps'] as List<dynamic>? ?? const [])
            .map(
              (value) =>
                  RecipeStep.fromJson(Map<String, dynamic>.from(value as Map)),
            )
            .toList(),
    review:
        json['latestReview'] is Map
            ? RecipeReview.fromJson(
              Map<String, dynamic>.from(json['latestReview'] as Map),
            )
            : null,
  );
}
...
class RecipeIngredient {
  ...
  final String name, unit, preparationNote;
  final num? amount;
  factory RecipeIngredient.fromJson(Map<String, dynamic> json) =>
      RecipeIngredient(
        ...
        unit: '${json['unit'] ?? ''}',
        preparationNote: '${json['preparationNote'] ?? ''}',
      );
...
class RecipeStep {
  const RecipeStep(this.instruction, {this.title = ''});
...
class RecipeReview {
  const RecipeReview(this.status, this.summary, this.feedback);
  final String status, summary, feedback;
