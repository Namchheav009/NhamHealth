...
class AiFoodView extends GetView<AiFoodController> {
                ...
                children: [
                  ...
                  Flexible(
                    child: SingleChildScrollView(
                      ...
                      child: _amountSheetBody(sheetContext),
                    ),
                  ),
                  _amountSheetActions(sheetContext),
                ],
              ),
            ),
          ),
  ...
  Widget _amountSheetBody(BuildContext context) {
    final isDrink = controller.inputKind.value == AiFoodInputKind.drink;
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'wellness.amount_before_analysis_help'.tr,
        ...
        AnimatedSwitcher(
          ...
          child:
              isDrink
                  ? _drinkAmountFields(context)
                  : _foodAmountFields(context),
        ),
        ...
        Container(
          ...
          child: Row(
            ...
            children: [
              ...
              Expanded(
                child: Text(
                  'wellness.nutrition_uses_selected_amount'.trParams({
                    'amount': controller.selectedAmountLabel,
                  }),
                  style: const TextStyle(
                    color: Color(0xFF31627D),
                    fontSize: 12,
                    height: 1.35,
                  ),
                ),
  ...
  Widget _amountSheetActions(BuildContext context) => Container(
  ...
  Widget _foodAmountFields(BuildContext context) => Column(
    key: const ValueKey<String>('food-amount-fields'),
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      Text(
        'wellness.amount'.tr,
      ...
      Row(
        children: [
          Expanded(child: _amountStepper(context)),
          const SizedBox(width: 10),
          Expanded(
            child: DropdownButtonFormField<String>(
              initialValue: controller.foodUnit.value,
                  ...
                  const ['plate', 'bowl', 'serving']
                      .map(
                        (unit) => DropdownMenuItem(
                          value: unit,
                          child: Text('wellness.$unit'.trOrSelf),
                        ),
                      )
                      .toList(),
              onChanged: (value) {
                if (value != null) controller.setFoodUnit(value);
              },
            ),
          ),
        ],
      ...
      Row(
        children:
            const [.25, .5, 1.0, 1.5, 2.0]
                .map(
                  (amount) => Expanded(
                    child: Padding(
                      padding: EdgeInsets.only(right: amount == 2 ? 0 : 7),
                      child: _amountChip(
                        context,
                        label:
                            amount == .25
                        ...
                        selected: controller.foodAmount.value == amount,
                        onSelected: () => controller.setFoodAmount(amount),
                      ),
                    ),
                  ),
                )
  ...
  Widget _drinkAmountFields(BuildContext context) {
    final percentage = (controller.drinkConsumedFraction.value * 100).round();
    return Column(
      key: const ValueKey<String>('drink-amount-fields'),
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(
        ...
        Row(
          children:
              const [250.0, 350.0, 500.0]
                  .map(
                    (size) => Expanded(
                      child: Padding(
                        padding: EdgeInsets.only(right: size == 500 ? 0 : 8),
                        child: _amountChip(
                          context,
                          label: '${size.toStringAsFixed(0)} ml',
                          selected: controller.drinkCupMl.value == size,
                          onSelected: () => controller.setDrinkCupMl(size),
                        ),
                      ),
                    ),
                  )
        ...
        SliderTheme(
          data: SliderTheme.of(context).copyWith(
            ...
            thumbColor: green,
            overlayColor: green.withValues(alpha: .12),
            thumbShape: const RoundSliderThumbShape(enabledThumbRadius: 7),
          ),
          child: Slider(
            value: controller.drinkConsumedFraction.value,
            min: .25,
            max: 1,
            divisions: 3,
            onChanged: controller.setDrinkConsumedFraction,
          ),
        ),
        Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
  ...
  Widget _amountStepper(BuildContext context) => Container(
    height: 54,
    decoration: BoxDecoration(
    ...
    child: Row(
      children: [
        _stepperButton(
          ...
          onTap:
              controller.foodAmount.value <= .25
                  ? null
                  : () => controller.adjustFoodAmount(-.25),
        ),
        Expanded(
          child: Text(
            controller.selectedAmountLabel.split(' ').first,
        ...
        _stepperButton(
          ...
          onTap: () => controller.adjustFoodAmount(.25),
        ),
      ],
    ),
  );
  ...
  Widget _stepperButton(
    ...
    icon: Icon(
      icon,
      size: 19,
      color: onTap == null ? context.appMutedText : greenDark,
    ),
  );
  ...
  Widget _amountChip(
    ...
    color: selected ? context.appSoftGreen : context.appSurfaceLow,
    borderRadius: BorderRadius.circular(10),
    child: InkWell(
      onTap: onSelected,
      borderRadius: BorderRadius.circular(10),
