const Map<String, String> wellnessKm = {
  'wellness.percent_left': 'នៅសល់ @value%',
  'wellness.meal_nutrition_summary':
      '@amount @unit • @calories kcal • ប្រូតេអ៊ីន @protein ក្រាម',
  'wellness.water_glass_summary':
      '@water មីលីលីត្រ • @glasses × 250 មីលីលីត្រក្នុងមួយកែវ',
  'wellness.grams_protein': 'ក្រាម ប្រូតេអ៊ីន',
  'wellness.about_total_ml':
      'ប្រហែល @total មីលីលីត្រ ដោយមិនរាប់បញ្ចូលទឹកកក និងពពុះដែលមើលឃើញ',
  'wellness.review_estimate_help':
      'ព័ត៌មានអាហារូបត្ថម្ភ និងបរិមាណពី AI គឺជាការប៉ាន់ស្មាន។ សូមពិនិត្យព័ត៌មានមុនពេលបន្ថែម។',
  'wellness.water_added': 'បានបន្ថែមទឹក',
  'wellness.food_added_success': 'បានបន្ថែម @name ដោយជោគជ័យ។',
  'wellness.correction_review_estimate':
      'ការកែតម្រូវរបស់អ្នកត្រូវបានផ្ញើឱ្យអ្នកគ្រប់គ្រងពិនិត្យ។ ព័ត៌មានអាហារូបត្ថម្ភនៅតែជាការប៉ាន់ស្មានរបស់ AI រហូតដល់អាហារនេះត្រូវបានបន្ថែមទៅមូលដ្ឋានទិន្នន័យ។',
  'wellness.correction_review_unit':
      'ការកែតម្រូវរបស់អ្នកត្រូវបានផ្ញើឱ្យអ្នកគ្រប់គ្រងពិនិត្យ។ ប្រើ @unit ដើម្បីគណនាអាហារូបត្ថម្ភឡើងវិញដោយត្រឹមត្រូវ។',
  'wellness.food_count_added_one': 'បានបន្ថែមអាហារ 1 មុខទៅថ្ងៃនេះ។',
  'wellness.food_count_added_many': 'បានបន្ថែមអាហារ @count មុខទៅថ្ងៃនេះ។',
  'wellness.water_count_added_one': 'បានបន្ថែមទឹក 1 កែវ។',
  'wellness.water_count_added_many': 'បានបន្ថែមទឹក @count កែវ។',
  'wellness.review_result': 'សូមពិនិត្យលទ្ធផលនេះ',
  'wellness.uncertain_result_help':
      'AI មិនទាន់ប្រាកដពេញលេញអំពីអាហារ ឬបរិមាណទេ។ សូមបញ្ជាក់ ឬកែតម្រូវលទ្ធផល មុនបន្ថែមទៅថ្ងៃនេះ។',
  'wellness.ai_food_search_assistant': 'ជំនួយការស្វែងរកអាហារដោយ AI',
  'wellness.water_progress': '@current / @target កែវ',
  'wellness.glass_remaining': 'នៅសល់ @count កែវ',
  'wellness.glasses_remaining': 'នៅសល់ @count កែវ',
  'wellness.selected_water_amount_one': '១ កែវ · @milliliters ml',
  'wellness.selected_water_amount_many': '@count កែវ · @milliliters ml',
  'wellness.nutrient_target': '/@target @unit',
  'wellness.nutrient_target_max': '/ គោលដៅអតិបរមា @target @unit',
  'wellness.carbohydrates': 'កាបូអ៊ីដ្រាត',
  'wellness.carbohydrates_detail_page': 'ព័ត៌មានកាបូអ៊ីដ្រាត',
  'wellness.daily_title': 'សុខភាពប្រចាំថ្ងៃ',
  'wellness.your_daily_summary': 'សង្ខេបសុខភាពថ្ងៃនេះ',
  'wellness.fiber_detail_page': 'ព័ត៌មានជាតិសរសៃ',
  'wellness.protein_detail_page': 'ព័ត៌មានប្រូតេអ៊ីន',
  'wellness.sugar_detail_page': 'ព័ត៌មានជាតិស្ករ',
  'wellness.choose_amount': 'ជ្រើសរើសបរិមាណ',
  'wellness.1_glass_is_about_250_ml': '១ កែវ ប្រហែល ២៥០ មីលីលីត្រ',
  'wellness.glass': 'កែវ',
  'wellness.glasses_remaining_fragment': 'កែវទៀតដើម្បីដល់គោលដៅ',
  'wellness.water_unavailable': 'មិនអាចប្រើមុខងារទឹកបាន',
  'wellness.unable_to_load_water_data': 'មិនអាចទាញយកទិន្នន័យទឹកបានទេ។',
  'wellness.unable_to_add_water_please_try_again':
      'មិនអាចបន្ថែមទឹកបានទេ។ សូមព្យាយាមម្តងទៀត។',
  'wellness.ai_food_check': 'ពិនិត្យអាហារដោយ AI',
  'wellness.take_photo': 'ថតរូប',
  'wellness.gallery': 'ជ្រើសរូបភាព',
  'wellness.analyze_food_or_drink': 'វិភាគអាហារ ឬភេសជ្ជៈ',
  'wellness.tell_us_the_amount': 'កំណត់បរិមាណអាហារ',
  'wellness.amount_before_analysis_help':
      'កែសម្រួលបរិមាណដែលអ្នកបានទទួលទាន ដើម្បីទទួលបានទិន្នន័យអាហារូបត្ថម្ភកាន់តែច្បាស់លាស់។',
  'wellness.amount_to_analyze': 'បរិមាណត្រូវវិភាគ៖ @amount',
  'wellness.quick_select': 'ជ្រើសរើសរហ័ស',
  'wellness.plate': 'ចាន',
  'wellness.bowl': 'ចានគោម',
  'wellness.cup_size': 'ទំហំពែង',
  'wellness.how_much_did_you_drink': 'តើអ្នកបានទទួលទានប៉ុន្មាន?',
  'wellness.edit_food_amount': 'កែសម្រួលបរិមាណអាហារ',
  'wellness.edit_drink_amount': 'កែសម្រួលបរិមាណភេសជ្ជៈ',
  'wellness.adjust_amount_actually_ate':
      'កែសម្រួលបរិមាណដែលអ្នកបានទទួលទានជាក់ស្តែង។',
  'wellness.select_cup_size_and_drink_amount':
      'ជ្រើសរើសទំហំពែង និងបរិមាណដែលបានទទួលទាន។',
  'wellness.preview_estimated': 'លទ្ធផលប៉ាន់ស្មាន',
  'wellness.volume_to_analyse': 'បរិមាណដែលត្រូវវិភាគ',
  'wellness.portion_of_cup': '@percent% នៃ @cup មីលីលីត្រ',
  'wellness.sugar_level': 'កម្រិតជាតិស្ករ',
  'wellness.sugar_percentage': 'ជាតិស្ករ @percent%',
  'wellness.custom': 'កំណត់ផ្ទាល់ខ្លួន',
  'wellness.enter_custom_cup_size': 'បញ្ចូលចំណុះពែងផ្ទាល់ខ្លួន (មីលីលីត្រ)',
  'wellness.nutrition_recalculated_notice':
      'តម្លៃអាហារូបត្ថម្ភនឹងត្រូវគណនាឡើងវិញ ដោយផ្អែកលើបរិមាណថ្មី។',
  'wellness.nutrition_recalculated_drink_notice':
      'តម្លៃអាហារូបត្ថម្ភនឹងត្រូវគណនាឡើងវិញ ផ្អែកលើចំណុះ @volume។',
  'wellness.volume_to_analyze': 'បរិមាណដែលត្រូវវិភាគ',
  'wellness.nutrition_uses_selected_amount':
      'អាហារូបត្ថម្ភនឹងត្រូវគណនាតាមបរិមាណ @amount ដែលបានជ្រើសរើស។',
  'wellness.analyze_this_amount': 'វិភាគបរិមាណនេះ',
  'wellness.update_amount': 'រក្សាទុកបរិមាណ',
  'wellness.continue_analysis': 'បន្តការវិភាគ',
  'wellness.added_to_today': 'បានបន្ថែមសម្រាប់ថ្ងៃនេះ',
  'wellness.water_added_today': 'បានបន្ថែមទឹកសម្រាប់ថ្ងៃនេះ',
  'wellness.adding': 'កំពុងបន្ថែម...',
  'wellness.add_to_todays_food': 'បន្ថែមទៅអាហារថ្ងៃនេះ',
  'wellness.add_to_todays_water': 'បន្ថែមទៅទឹកថ្ងៃនេះ',
  'wellness.know_what_you_eat_or_drink': 'ស្គាល់អ្វីដែលអ្នកញ៉ាំ និងផឹក',
  'wellness.snap_a_clear_photo_for_instant_nutrition_insights':
      'ថតរូបអាហារឱ្យច្បាស់ ដើម្បីទទួលព័ត៌មានអាហារូបត្ថម្ភភ្លាមៗ។',
  'wellness.use_camera_or_choose_from_gallery':
      'ថតរូបថ្មី ឬជ្រើសរូបភាពពីទូរសព្ទ',
  'wellness.confidence': 'ភាពជឿជាក់របស់ AI',
  'wellness.nutrition_estimate': 'អាហារូបត្ថម្ភប៉ាន់ស្មាន',
  'wellness.checking_image_quality': 'កំពុងពិនិត្យគុណភាពរូបភាព',
  'wellness.recognizing_food_and_drinks': 'កំពុងស្គាល់អាហារ និងភេសជ្ជៈ',
  'wellness.estimating_portions': 'កំពុងប៉ាន់ស្មានបរិមាណ',
  'wellness.calculating_nutrition_and_sugar':
      'កំពុងគណនាអាហារូបត្ថម្ភ និងជាតិស្ករ',
  'wellness.results_update_when_the_full_food_and_drink_check_is_complete':
      'លទ្ធផលនឹងបង្ហាញនៅពេលការពិនិត្យអាហារ និងភេសជ្ជៈបានបញ្ចប់។',
  'wellness.sugar_analysis': 'ការវិភាគជាតិស្ករ',
  'wellness.estimate': 'ការប៉ាន់ស្មាន',
  'wellness.estimated_total_sugar': 'ជាតិស្ករសរុបប៉ាន់ស្មាន',
  'wellness.about_teaspoons': 'ប្រហែលជាស្លាបព្រាកាហ្វេ',
  'wellness.sugar_share_of_carbs': 'សមាមាត្រជាតិស្ករក្នុងកាបូអ៊ីដ្រាត',
  'wellness.this_is_estimated_total_sugar_a_photo_cannot_reliably_separate_added_sugar_from_naturally_occurring_sugar_so_it_is_not_a_daily_value_percentage_check_the_package_label_or_recipe_when_available':
      'នេះជាជាតិស្ករសរុបដែលបានប៉ាន់ស្មាន។ រូបថតមិនអាចបែងចែកជាតិស្ករបន្ថែមពីជាតិស្ករធម្មជាតិបានច្បាស់លាស់ទេ ដូច្នេះវាមិនមែនជាភាគរយតម្លៃប្រចាំថ្ងៃទេ។ សូមពិនិត្យស្លាកផលិតផល ឬរូបមន្តនៅពេលមាន។',
  'wellness.food': 'អាហារ',
  'wellness.food_and_drink': 'អាហារ + ភេសជ្ជៈ',
  'wellness.plain_water_detected': 'AI បានរកឃើញថាជាទឹកសុទ្ធ',
  'wellness.drink_volume': 'បរិមាណភេសជ្ជៈ',
  'wellness.carbs': 'កាបូអ៊ីដ្រាត',
  'wellness.serving': 'ចំណែក',
  'wellness.review_this_estimate': 'ពិនិត្យលទ្ធផលប៉ាន់ស្មាន',
  'wellness.ai_nutrition_and_portions_are_estimates_check_the_details_before_adding':
      'អាហារូបត្ថម្ភ និងបរិមាណពី AI គឺជាការប៉ាន់ស្មាន។ សូមពិនិត្យព័ត៌មានលម្អិតមុនពេលបន្ថែមចូលថ្ងៃនេះ។',
  'wellness.important_information': 'ព័ត៌មានសំខាន់',
  'wellness.ai_nutrition_results_are_estimates_for_general_wellness_only_they_are_not_medical_advice_a_diagnosis_or_an_official_nutrition_label':
      'លទ្ធផលអាហារូបត្ថម្ភពី AI គ្រាន់តែជាការប៉ាន់ស្មានសម្រាប់សុខុមាលភាពទូទៅ។ វាមិនមែនជាដំបូន្មានវេជ្ជសាស្ត្រ ការធ្វើរោគវិនិច្ឆ័យ ឬស្លាកអាហារូបត្ថម្ភផ្លូវការទេ។',
  'wellness.food_photos_are_sent_to_the_configured_ai_provider_for_analysis_do_not_include_faces_documents_or_other_personal_information':
      'រូបថតអាហារត្រូវបានផ្ញើទៅកាន់អ្នកផ្តល់សេវា AI សម្រាប់វិភាគ។ កុំបញ្ចូលមុខ ឯកសារ ឬព័ត៌មានផ្ទាល់ខ្លួនផ្សេងទៀត។',
  'wellness.confirm_food_first': 'បញ្ជាក់អាហារមុន',
  'wellness.the_result_is_uncertain_confirm_the_food_or_retake_a_clear_well_lit_photo_before_saving':
      'លទ្ធផលមិនទាន់ច្បាស់ទេ។ សូមបញ្ជាក់អាហារ ឬថតរូបថ្មីឱ្យច្បាស់ និងមានពន្លឺល្អ មុនពេលរក្សាទុក។',
  'wellness.consider_a_smaller_portion': 'សាកល្បងបន្ថយបរិមាណ',
  'wellness.about_calories_kcal_exceeds_your_remaining_remaining_kcal_target_reduce_the_portion_or_save_it_for_tomorrow':
      'ប្រហែល @calories kcal លើសគោលដៅដែលនៅសល់ @remaining kcal របស់អ្នក។ សូមកាត់បន្ថយបរិមាណ ឬទុកសម្រាប់ថ្ងៃស្អែក។',
  'wellness.balance_the_sugar': 'រក្សាជាតិស្ករឱ្យមានតុល្យភាព',
  'wellness.estimated_sugar_is_sugar_g_pair_it_with_protein_or_fiber_and_choose_an_unsweetened_drink':
      'ជាតិស្ករប៉ាន់ស្មានមាន @sugar ក្រាម។ សូមញ៉ាំជាមួយប្រូតេអ៊ីន ឬជាតិសរសៃ ហើយជ្រើសភេសជ្ជៈគ្មានជាតិស្ករ។',
  'wellness.add_a_leaner_balance': 'បន្ថែមអាហារខ្លាញ់តិច',
  'wellness.this_looks_fat_heavy_for_its_protein_a_smaller_portion_with_vegetables_or_lean_protein_would_improve_balance':
      'អាហារនេះមានខ្លាញ់ច្រើនធៀបនឹងប្រូតេអ៊ីន។ សាកល្បងញ៉ាំតិចជាងមុន ហើយបន្ថែមបន្លែ ឬប្រូតេអ៊ីនខ្លាញ់តិច ដើម្បីឱ្យមានតុល្យភាពល្អ។',
  'wellness.strong_protein_choice': 'ជម្រើសប្រូតេអ៊ីនល្អ',
  'wellness.provides_about_protein_g_protein_and_fits_your_remaining_calories_add_vegetables_for_fiber':
      'ផ្តល់ប្រូតេអ៊ីនប្រហែល @protein ក្រាម និងសមនឹងកាឡូរីដែលនៅសល់។ សូមបន្ថែមបន្លែសម្រាប់ជាតិសរសៃ។',
  'wellness.fits_todays_plan': 'សមនឹងផែនការថ្ងៃនេះ',
  'wellness.about_calories_kcal_fits_your_target_keep_the_portion_near_servingsize_servingunit':
      'ប្រហែល @calories kcal សមនឹងគោលដៅរបស់អ្នក។ គួរញ៉ាំប្រហែល @servingSize @servingUnit។',
  'wellness.database_calculated': 'គណនាពីទិន្នន័យអាហារ',
  'wellness.database_and_ai_estimate': 'មូលដ្ឋានទិន្នន័យ + ការប៉ាន់ស្មានពី AI',
  'wellness.ai_estimate': 'ការប៉ាន់ស្មានពី AI',
  'wellness.partial_nutrition': 'ព័ត៌មានអាហារូបត្ថម្ភមិនពេញលេញ',
  'wellness.user_entered': 'បញ្ចូលដោយអ្នក',
  'wellness.nutrition_unavailable': 'មិនអាចគណនាអាហារូបត្ថម្ភបាន',
  'wellness.database_matched': 'បានផ្គូផ្គងជាមួយទិន្នន័យអាហារ',
  'wellness.ai_meal_auto_fill': 'បំពេញបញ្ជីអាហារដោយ AI',
  'wellness.what_did_you_eat': 'អ្នកបានញ៉ាំអ្វី?',
  'wellness.example_150_g_chicken_breast_1_cup_rice_banana':
      'ឧទាហរណ៍៖ សាច់មាន់ ១៥០ ក្រាម បាយ ១ ពែង ចេក',
  'wellness.matching_foods': 'កំពុងស្វែងរកអាហារដែលត្រូវគ្នា...',
  'wellness.create_meal_draft': 'បង្កើតបញ្ជីអាហារព្រាង',
  'wellness.add_all_to_today': 'បន្ថែមទាំងអស់ទៅថ្ងៃនេះ',
  'wellness.g_protein': 'ក្រាមប្រូតេអ៊ីន',
  'wellness.foods': 'មុខអាហារ',
  'wellness.food_names_are_matched_to_nhamhealths_nutrition_catalog_review_quantities_before_saving_results_are_for_general_wellness_only':
      'ឈ្មោះអាហារត្រូវបានផ្គូផ្គងជាមួយបញ្ជីទិន្នន័យអាហារូបត្ថម្ភរបស់ NhamHealth។ សូមពិនិត្យបរិមាណមុនពេលរក្សាទុក; លទ្ធផលនេះសម្រាប់សុខភាពទូទៅប៉ុណ្ណោះ។',
  'wellness.review_before_logging': 'ពិនិត្យមុនកត់ត្រា',
  'wellness.adding_meal': 'កំពុងបន្ថែមអាហារ...',
  'wellness.log_a_whole_meal_in_one_step': 'កត់ត្រាអាហារមួយពេលក្នុងជំហានតែមួយ',
  'wellness.separate_foods_with_commas_or_write_one_per_line_include_amounts_when_you_know_them':
      'បំបែកអាហារដោយសញ្ញាក្បៀស ឬសរសេរមួយមុខក្នុងមួយបន្ទាត់។ បញ្ចូលបរិមាណប្រសិនបើអ្នកដឹង។',
  'wellness.add_your_meal': 'បន្ថែមអាហាររបស់អ្នក',
  'wellness.enter_one_or_more_foods_first': 'សូមបញ្ចូលអាហារមួយ ឬច្រើនមុខជាមុន។',
  'wellness.no_catalog_matches': 'រកមិនឃើញអាហារក្នុងបញ្ជីទិន្នន័យ',
  'wellness.try_simpler_food_names_separated_by_commas':
      'សាកល្បងឈ្មោះអាហារសាមញ្ញ ហើយបំបែកដោយសញ្ញាក្បៀស។',
  'wellness.could_not_analyze_meal': 'មិនអាចវិភាគអាហារបានទេ',
  'wellness.meal_added': 'បានបន្ថែមអាហារ',
  'wellness.could_not_save_meal': 'មិនអាចរក្សាទុកអាហារបានទេ',
  'wellness.please_try_again': 'សូមព្យាយាមម្តងទៀត។',
  'wellness.add_food_source': 'បន្ថែមប្រភពអាហារ',
  'wellness.meal_type': 'ប្រភេទអាហារ',
  'wellness.breakfast': 'អាហារពេលព្រឹក',
  'wellness.lunch': 'អាហារថ្ងៃត្រង់',
  'wellness.dinner': 'អាហារពេលល្ងាច',
  'wellness.drink': 'ភេសជ្ជៈ',
  'wellness.snack': 'អាហារសម្រន់',
  'wellness.example_chicken_rice': 'ឧទាហរណ៍៖ បាយសាច់មាន់',
  'wellness.invalid_information': 'ព័ត៌មានមិនត្រឹមត្រូវ',
  'wellness.please_enter_food_name_and_calories':
      'សូមបញ្ចូលឈ្មោះអាហារ និងកាឡូរី។',
  'wellness.add_food': 'បន្ថែមអាហារ',
  'wellness.ai_insight': 'ការណែនាំពី AI',
  'wellness.needs_balance': 'គួររក្សាតុល្យភាព',
  'wellness.sugar_is_a_bit_high_and_fiber_is_still_low_drink_1_more_glass_of_water_and_choose_a_light_next_meal':
      'ជាតិស្ករខ្ពស់បន្តិច ហើយជាតិសរសៃនៅតិច។ សាកល្បងផឹកទឹកមួយកែវទៀត និងជ្រើសអាហារបន្ទាប់ដែលស្រាលជាងមុន។',
  'wellness.add_fruit': 'បន្ថែមផ្លែឈើ',
  'wellness.drink_more_water': 'ផឹកទឹកបន្ថែម',
  'wellness.choose_lighter_dinner': 'ជ្រើសអាហារពេលល្ងាចស្រាលជាងមុន',
  'wellness.needs_balance_2': '⚖️ គួររក្សាតុល្យភាព',
  'wellness.most_of_your_calories_came_from_food_and_sweet_drinks_stay_balanced_by_choosing_a_lighter_next_meal':
      'កាឡូរីភាគច្រើនថ្ងៃនេះមកពីអាហារ និងភេសជ្ជៈផ្អែម។ សាកល្បងជ្រើសអាហារបន្ទាប់ដែលស្រាលជាងមុន ដើម្បីរក្សាតុល្យភាព។',
  'wellness.add_fruit_2': '🍎 បន្ថែមផ្លែឈើ',
  'wellness.choose_lighter_dinner_2': '🥗 ជ្រើសអាហារពេលល្ងាចស្រាលជាងមុន',
  'wellness.log_food_with_ai': 'កត់ត្រាអាហារដោយ AI',
  'wellness.tell_ai_what_you_ate_and_choose_the_amount_for_a_better_estimate':
      'ប្រាប់ AI អំពីអាហារដែលអ្នកបានញ៉ាំ និងជ្រើសបរិមាណ ដើម្បីប៉ាន់ស្មានបានកាន់តែប្រសើរ។',
  'wellness.open_ai_meal_auto_fill': 'បើកបំពេញបញ្ជីអាហារដោយ AI',
  'wellness.edit_intake': 'កែសម្រួលបរិមាណទទួលទាន',
  'wellness.youre_on_track_today': '✨ ថ្ងៃនេះអ្នកកំពុងធ្វើបានល្អ',
  'wellness.edit_amount': 'កែសម្រួលបរិមាណ',
  'wellness.re_analyze_with_ai': 'វិភាគឡើងវិញជាមួយ AI',
  'wellness.hide_manual_edit': 'លាក់ការកែសម្រួលដោយផ្ទាល់',
  'wellness.edit_manually': 'កែសម្រួលដោយផ្ទាល់',
  'wellness.edit_amount_manually': 'កែសម្រួលបរិមាណដោយផ្ទាល់',
  'wellness.this_drink_added_a_lot_of_sugar_with_little_fiber_or_protein_main_source_of_sugar_today_pairs_better_with_water_or_fruit':
      'ភេសជ្ជៈនេះមានជាតិស្ករច្រើន ប៉ុន្តែមានជាតិសរសៃ និងប្រូតេអ៊ីនតិច។\n• ជាប្រភពជាតិស្ករចម្បងសម្រាប់ថ្ងៃនេះ\n• គួរពិសាទឹកបរិសុទ្ធ ឬផ្លែឈើបន្ថែម',
  'wellness.contribution_today': 'ចំណែកក្នុងថ្ងៃនេះ',
  'wellness.watch_sugar': '⚖️ ប្រយ័ត្នជាតិស្ករ',
  'wellness.milk_tea_is_okay_sometimes_but_sugar_is_a_bit_high_for_one_drink_balance_it_with_water_and_a_lighter_next_choice':
      'តែទឹកដោះគោអាចផឹកម្តងម្កាលបាន ប៉ុន្តែមានជាតិស្ករខ្ពស់បន្តិច។ សាកល្បងផឹកទឹកបន្ថែម និងជ្រើសអាហារបន្ទាប់ដែលស្រាលជាងមុន។',
  'wellness.medium_cup_added_10_30_am':
      '☕ ពែងមធ្យម   ⏰ បានបន្ថែមម៉ោង ១០:៣០ ព្រឹក',
  'wellness.sweet_drink_enjoy_in_balance':
      'ភេសជ្ជៈផ្អែម—គួរផឹកក្នុងកម្រិតសមរម្យ។',
  'wellness.of_todays_calories': 'នៃកាឡូរី\nថ្ងៃនេះ',
  'wellness.hydration_tip': 'គន្លឹះផឹកទឹក',
  'wellness.todays_food_sources': 'អាហារដែលបានកត់ត្រាថ្ងៃនេះ',
  'wellness.food_not_found': 'រកមិនឃើញអាហារ',
  'wellness.please_select_a_food_source_again': 'សូមជ្រើសប្រភពអាហារម្តងទៀត។',
  'wellness.ai_re_analysis': 'ការវិភាគឡើងវិញដោយ AI',
  'wellness.re_analyzing_your_food_amount':
      'កំពុងវិភាគបរិមាណអាហាររបស់អ្នកឡើងវិញ...',
  'wellness.food_detail_changes_saved_successfully':
      'បានរក្សាទុកការកែប្រែព័ត៌មានអាហារ។',
  'wellness.wellness_unavailable': 'មិនអាចបង្ហាញទិន្នន័យសុខភាពបាន',
  'wellness.unable_to_load_your_daily_wellness_data':
      'មិនអាចផ្ទុកទិន្នន័យសុខភាពប្រចាំថ្ងៃបាន។',
  'wellness.food_analysis_failed_please_try_another_photo':
      'មិនអាចវិភាគអាហារបានទេ។ សូមសាកល្បងថតរូបថ្មី ឬប្តូររូបថតផ្សេង។',
  'wellness.food_added': 'បានបន្ថែមអាហារ',
  'wellness.could_not_save_food': 'មិនអាចរក្សាទុកអាហារបានទេ',
  'wellness.could_not_save_correction': 'មិនអាចរក្សាទុកការកែតម្រូវបានទេ',
  'wellness.could_not_save_feedback': 'មិនអាចរក្សាទុកការបញ្ជាក់បានទេ',
  'wellness.your_nutrition_was_not_stored_please_check_the_server_and_try_again':
      'មិនអាចរក្សាទុកព័ត៌មានអាហារូបត្ថម្ភបានទេ។ សូមពិនិត្យការតភ្ជាប់ ហើយព្យាយាមម្តងទៀត។',
  'wellness.food_confirmed': 'បានបញ្ជាក់អាហារ',
  'wellness.thanks_your_confirmation_helps_improve_future_results':
      'សូមអរគុណ—ការបញ្ជាក់របស់អ្នកជួយកែលម្អលទ្ធផលនាពេលអនាគត។',
  'wellness.correction_saved': 'បានរក្សាទុកការកែតម្រូវ',
  'wellness.nutrition_was_recalculated_from_the_database':
      'អាហារូបត្ថម្ភត្រូវបានគណនាឡើងវិញពីមូលដ្ឋានទិន្នន័យ។',
  'wellness.your_calorie_changes_have_been_saved':
      'ការផ្លាស់ប្តូរកាឡូរីរបស់អ្នកត្រូវបានរក្សាទុក។',
  'wellness.tap_reload_below_to_try_again':
      'ចុច «ផ្ទុកឡើងវិញ» ខាងក្រោម ដើម្បីព្យាយាមម្តងទៀត។',
  'wellness.unverified': 'មិនទាន់ផ្ទៀងផ្ទាត់',
  'wellness.phone_verification': 'ការផ្ទៀងផ្ទាត់លេខទូរស័ព្ទ',
  'wellness.enter_the_6_digit_code_sent_to_your_phone':
      'បញ្ចូលលេខកូដ ៦ ខ្ទង់ដែលបានផ្ញើទៅទូរស័ព្ទរបស់អ្នក។',
  'wellness.verification_code_sent_to_your_phone':
      'លេខកូដផ្ទៀងផ្ទាត់ត្រូវបានផ្ញើទៅកាន់ទូរស័ព្ទរបស់អ្នក។',
  'wellness.sending_verification_code': 'កំពុងផ្ញើលេខកូដផ្ទៀងផ្ទាត់...',
  'wellness.correct': 'កែតម្រូវ',
  'wellness.correct_ai_result': 'កែតម្រូវលទ្ធផល AI',
  'wellness.correction_description':
      'កែឈ្មោះអាហារ និងបរិមាណ ដើម្បីគណនាអាហារូបត្ថម្ភឡើងវិញ។',
  'wellness.food_name': 'ឈ្មោះអាហារ',
  'wellness.serving_details': 'ព័ត៌មានបរិមាណទទួលទាន',
  'wellness.amount': 'បរិមាណ',
  'wellness.unit': 'ឯកតា',
  'wellness.unit_examples': 'ឧទាហរណ៍៖ g, ml, ចាន, ពែង ឬមួយចំណែក',
  'wellness.save_correction': 'រក្សាទុកការកែតម្រូវ',
  'wellness.analysis_empty': 'លទ្ធផលវិភាគអាហារូបត្ថម្ភរបស់អ្នកនឹងបង្ហាញនៅទីនេះ',
  'wellness.choose_photo_to_analyze':
      'ជ្រើសរើសរូបថតអាហារ ឬភេសជ្ជៈដែលច្បាស់ រួចចុច «វិភាគ»។',
  'wellness.fat_detail': 'ព័ត៌មានលម្អិតអំពីជាតិខ្លាញ់',
};
